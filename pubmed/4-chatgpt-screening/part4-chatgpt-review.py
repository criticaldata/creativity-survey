"""
Part 4: ChatGPT API-Based Review of Creative AI Papers

This script uses the OpenAI API to review PDF papers for inclusion in a
...
MODEL = "gpt-5.2"
# Input/Output paths
BASE_DIR = Path("/home/sebasmos/Desktop/AnpassenNN/mit-projects/creative-ai/code-pubmed-vertex/part-4-review-chatgpt")
INPUT_CSV = BASE_DIR / "papers-needs-review-blinded.csv"
OUTPUT_CSV = BASE_DIR / "papers-reviewed-chatgpt.csv"
PDF_DIR = BASE_DIR / "papers-20251229T104717Z-1-001" / "papers"
...
def find_pdf_file(pdf_filename: str, pdf_dir: Path) -> Path:
    ...
    for pdf_path in all_pdfs:
        ...
        # Match pattern like "fnins-19-1606798" or "fphar-12-739053"
        ...
        if csv_match and file_match:
            ...
            # Check if volume matches and article numbers are similar
            if csv_vol == file_vol:
                # Check substring or high similarity of article number
...
def extract_year_from_text(text: str) -> int:
    ...
    import re
    # Look for copyright year or publication date patterns
    patterns = [
        r'©\s*(\d{4})',
        r'Copyright\s*©?\s*(\d{4})',
        r'Published:?\s*\w+\s+\d{1,2},?\s*(\d{4})',
        r'Received:?\s*\w+\s+\d{1,2},?\s*(\d{4})',
        r'\b(20[12][0-9])\b'  # Any year 2010-2029
    ]
...
def create_review_prompt(title: str, abstract: str, full_text: str) -> str:
    """Create the prompt for ChatGPT to review a paper."""
    # Truncate full text to avoid token limits (keep first ~15000 chars)
...
DECISION GUIDANCE:
- ACCEPT: Relevant to any of the 5 thematic clusters, studies creativity/cognition, proposes frameworks, or provides useful insights for understanding creative AI
- MAYBE: Partially relevant, might provide useful background or tangential insights
- REJECT: Completely off-topic (e.g., purely clinical drug trials with no creativity/cognition focus, unrelated technical methods)
...
def review_paper_with_gpt(client: OpenAI, title: str, abstract: str, full_text: str) -> dict:
    """Use GPT to review a single paper."""
...
def main():
    print("=" * 70)
    print("Part 4: ChatGPT API-Based Review of Creative AI Papers")
    print(f"Model: {MODEL}")
    print("=" * 70)
    ...
    for idx, row in df.iterrows():
        ...
        if is_mdpi_paper(pdf_filename):
            df.at[idx, 'Decision-chatgpt'] = 'Reject'
            df.at[idx, 'comment-chatgpt'] = 'MDPI publication - excluded per criteria'
            stats["MDPI_excluded"] += 1
            stats["Reject"] += 1
            print(f"  -> REJECTED (MDPI publication)")
            continue

        # Find and read PDF (with fuzzy matching for typos)
        pdf_path = find_pdf_file(pdf_filename, PDF_DIR)

        if pdf_path is None:
            df.at[idx, 'Decision-chatgpt'] = 'Maybe'
            df.at[idx, 'comment-chatgpt'] = 'PDF file not found - needs manual review'
        ...
        if full_text.startswith("ERROR"):
            df.at[idx, 'Decision-chatgpt'] = 'Maybe'
            df.at[idx, 'comment-chatgpt'] = f'PDF extraction error - needs manual review'
            stats["Maybe"] += 1
        ...
        if year and year < 2020:
            df.at[idx, 'Decision-chatgpt'] = 'Reject'
            df.at[idx, 'comment-chatgpt'] = f'Pre-2020 publication ({year}) - excluded per criteria'
        ...
        result = review_paper_with_gpt(client, title, abstract, full_text)

        decision = result.get('decision', 'Maybe')
        comment = result.get('comment', '')[:100]  # Limit to 100 chars

        # Add flags for editorials and background material (but don't change decision)
        flags = []
